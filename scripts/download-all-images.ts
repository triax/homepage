...
class ProgressTracker {
  ...
  display(): void {
    ...
    if (CONFIG.GITHUB_ACTIONS) {
      ...
      const statusLine =
        `\rProgress: ${progressBar} ${percentage}% [${progress}/${this.total}] ` +
...
async function createDownloadTask(
  ...
  try {
    ...
    if (CONFIG.GITHUB_ACTIONS) {
      console.log(
        `::error::Failed to download ${memberName} ${photoType}: ${(error as Error).message}`
      );
    }
...
async function downloadImagesInParallel(
  ...
  for (let i = 0; i < downloadTasks.length; i += CONFIG.CONCURRENT_DOWNLOADS) {
    ...
    const batchResults = await Promise.all(batch.map((task) => createDownloadTask(task, progress)));
