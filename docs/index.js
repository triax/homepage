...
function photoAlt(member, photo) {
  return photo === PLACEHOLDER_PHOTO
    ? `${member.name.default}（写真準備中）`
    : member.name.default;
}
...
function createMemberCard(member) {
  ...
  const jersey = hasNumber(member)
    ? `<div class="absolute top-2 right-2 bg-black/70 text-white px-2 py-1 rounded text-sm z-10">
                            #${member.number}
                        </div>`
    : "";
  const info = `
...
function filterByPosition(position) {
  ...
  document.querySelectorAll(".position-filter").forEach((btn) => {
    if (btn.dataset.position === position) {
      btn.className =
        "position-filter px-4 py-2 rounded-full bg-red-600 text-white";
    } else {
      btn.className =
        "position-filter px-4 py-2 rounded-full bg-gray-200 hover:bg-gray-300";
    }
  });
}
...
function displayMembers() {
  ...
  const filteredMembers =
    currentPosition === "ALL"
      ? allMembers
      : allMembers.filter(
          (m) => (m.position || "").toUpperCase() === currentPosition,
        );
...
function displayRandomMemberPickup(members) {
  ...
  if (infoContainer) {
    ...
    // 値が存在するオプションのみフィルター
    const availableOptions = infoOptions.filter(
      (option) => randomMember[option.key],
    );
    ...
    if (availableOptions.length > 0) {
      ...
      if (labelElement && valueElement) {
        ...
        const infoContainer = document.getElementById(
          "pickup-member-pickup-info",
        );
        infoContainer.className =
          "relative mt-4 bg-white/95 backdrop-blur text-gray-800 px-8 py-4 rounded-2xl shadow-xl max-w-md mx-auto";
        ...
        valueElement.className =
          "text-base md:text-xl font-bold text-gray-900 leading-relaxed";
      }
    } else {
      ...
      if (valueElement) {
        ...
        valueElement.className =
          "text-3xl md:text-5xl font-bold mb-4 text-center";
      }
    }
  }
}
...
function setupHeroVideo() {
  ...
  video.addEventListener("playing", () => video.classList.remove("opacity-0"), {
    once: true,
  });
...
async function initInstagram() {
  ...
  (data?.posts || []).forEach((post) => {
    ...
    document
      ...
      .insertAdjacentHTML("beforeend", postItem);

    // モバイル用
    const mobilePostItem = createInstagramPostItemMobile(post);
    document
      .getElementById("instagram-feed-mobile")
      .insertAdjacentHTML("beforeend", mobilePostItem);
  });
  ...
  document
    .getElementById("instagram-feed-mobile")
    .insertAdjacentHTML("beforeend", moreCard);

  // Instagram要素に対してfade-inアニメーションを適用
  const instagramItems = document.querySelectorAll(
    "#instagram-feed .fade-in, #instagram-feed-mobile .fade-in",
  );
  const observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
...
function createInstagramPostItem({
  caption,
  media_type,
  permalink,
  timestamp,
...
function createInstagramPostItemMobile({
  caption,
  media_type,
  permalink,
  timestamp,
