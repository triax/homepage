...
function createSeasonHeader(title) {
  ...
  header.className =
    "text-lg font-semibold text-gray-700 text-center mb-4 mt-6 first:mt-0";
...
function createGameCard(game, ticketUrl, isOpen) {
  ...
  div.className =
    "bg-white rounded-lg shadow hover:shadow-lg transition-shadow fade-in";
  ...
  const winClass =
    result.win === true
      ...
      : result.win === false
        ...
        : "text-gray-600";
  const winLabel =
    result.win === true ? "WIN" : result.win === false ? "LOSE" : "DRAW";
